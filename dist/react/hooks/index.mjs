...
function useCacheValue(key, defaultValue, ttl = 0) {
  const cache = useService(ICache.$);
  const [value, setValue] = (0, import_react3.useState)(defaultValue);
  const [loading, setLoading] = (0, import_react3.useState)(true);
  const [error, setError] = (0, import_react3.useState)(null);
  const fetchValue = (0, import_react3.useCallback)(async () => {
    setLoading(true);
    setError(null);
    try {
  ...
  const setCacheValue = (0, import_react3.useCallback)(
  ...
  const removeCacheValue = (0, import_react3.useCallback)(async () => {
    setError(null);
    try {
...
function useCachedData(key, fetcher, ttl = 60, options = {}) {
  ...
  const [loading, setLoading] = (0, import_react4.useState)(!options.skipInitialFetch);
  const [error, setError] = (0, import_react4.useState)(null);
  const [lastUpdated, setLastUpdated] = (0, import_react4.useState)(null);
  ...
  const invalidate = (0, import_react4.useCallback)(async () => {
    await cache.forget(key);
    setData(null);
  }, [cache, key]);
...
function useMultiCacheValues(keys, defaultValues) {
  ...
  const fetchValues = (0, import_react5.useCallback)(async () => {
    setLoading(true);
    try {
  ...
  const setCacheValues = (0, import_react5.useCallback)(
    async (newValues, ttl) => {
      try {
  ...
  const removeCacheValues = (0, import_react5.useCallback)(async () => {
    try {
...
function useAllConfig(options = {}) {
  const config = useService(IConfig.$);
  const [allConfig, setAllConfig] = (0, import_react6.useState)(config.all());
...
function useConfig(key, defaultValue, options = {}) {
  const config = useService(IConfig.$);
  const [value, setValue] = (0, import_react7.useState)(config.get(key, defaultValue));
...
function useConfigState(key, defaultValue) {
  ...
  const setConfigValue = (0, import_react8.useCallback)(
    (newValue) => {
...
function useHasConfig(key) {
  const config = useService(IConfig.$);
  const [exists, setExists] = (0, import_react9.useState)(config.has(key));
...
var ContainerContext = (0, import_react10.createContext)(void 0);
...
export { useAllConfig, useApp, useCacheValue, useCachedData, useConfig, useConfigState, useContainer, useHasConfig, useHasTranslation, useICache, useLocale, useMultiCacheValues, useResolve, useService, useTranslate, useTranslator, withServices };
