...
function useConfig(key, defaultValue, options = {}) {
  ...
  const [value, setValue] = (0, import_react3.useState)(config.get(key, defaultValue));
...
function useHasConfig(key) {
  const config = useService(IConfig.$);
  const [exists, setExists] = (0, import_react4.useState)(config.has(key));
...
function useAllConfig(options = {}) {
  const config = useService(IConfig.$);
  const [allConfig, setAllConfig] = (0, import_react5.useState)(config.all());
...
function useConfigState(key, defaultValue) {
  const config = useService(IConfig.$);
  const [value, setValue] = (0, import_react6.useState)(config.get(key, defaultValue));
  const setConfigValue = (0, import_react6.useCallback)(
    (newValue) => {
...
export { useAllConfig, useConfig, useConfigState, useHasConfig };
